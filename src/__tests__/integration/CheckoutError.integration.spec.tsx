...
describe('Checkout.Error', () => {
  beforeEach(() => {
    ...
    window.history.replaceState({}, '', '/checkout/error');
  });
  ...
  it('should contain informations about payment error on the page', async () => {
    render(<App queryClient={queryClient} />);
    await waitAbsoluteLoader();

    const heading = screen.getByText(/^checkout\.error\.title$/i);
    const body = screen.getByText(/^checkout\.error\.body$/i);
